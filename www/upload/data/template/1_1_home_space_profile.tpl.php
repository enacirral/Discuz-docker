...
0
|| checktplrefresh('./template/default/home/space_profile.htm', './template/default/home/space_header.htm', 1700941935, '1', './data/template/1_1_home_space_profile.tpl.php', './template/default', 'home/space_profile')
|| checktplrefresh('./template/default/home/space_profile.htm', './template/default/home/space_profile_body.htm', 1700941935, '1', './data/template/1_1_home_space_profile.tpl.php', './template/default', 'home/space_profile')
|| checktplrefresh('./template/default/home/space_profile.htm', './template/default/home/space_userabout.htm', 1700941935, '1', './data/template/1_1_home_space_profile.tpl.php', './template/default', 'home/space_profile')
|| checktplrefresh('./template/default/home/space_profile.htm', './template/default/common/header_common.htm', 1700941935, '1', './data/template/1_1_home_space_profile.tpl.php', './template/default', 'home/space_profile')
|| checktplrefresh('./template/default/home/space_profile.htm', './template/default/home/space_diy.htm', 1700941935, '1', './data/template/1_1_home_space_profile.tpl.php', './template/default', 'home/space_profile')
|| checktplrefresh('./template/default/home/space_profile.htm', './template/default/home/space_header_personalnv.htm', 1700941935, '1', './data/template/1_1_home_space_profile.tpl.php', './template/default', 'home/space_profile')
;?>
...
<?php if($_G['group']['allowmanagearticle'] || $_G['group']['allowdiy']|| getstatus($_G['member']['allowadmincp'], 4) || getstatus($_G['member']['allowadmincp'], 2) || getstatus($_G['member']['allowadmincp'], 3) || (is_array($_G['setting']['ext_portalmanager']) && in_array($_G['uid'], $_G['setting']['ext_portalmanager']))) { ?><a href="portal.php?mod=portalcp">门户管理</a><?php } if($_G['uid'] && $_G['group']['radminid'] > 1) { ?><a href="forum.php?mod=modcp&amp;fid=<?php echo $_G['fid'];?>" target="_blank"><?php echo $_G['setting']['navs'][2]['navname'];?>管理</a><?php } if($_G['uid'] && ($_G['group']['radminid'] == 1 || getstatus($_G['member']['allowadmincp'], 1))) { ?><a href="admin.php" target="_blank">管理中心</a><?php } ?>
<?php if(!empty($_G['setting']['pluginhooks']['global_usernav_extra2'])) echo $_G['setting']['pluginhooks']['global_usernav_extra2'];?>
<a href="member.php?mod=logging&amp;action=logout&amp;formhash=<?php echo FORMHASH;?>">退出</a>
<?php if($space['self'] && $do == 'index') { ?><a id="diy-tg" href="javascript:openDiy();" title="装扮空间">装扮空间</a><?php } ?>
</div>
...
<div class="y">
<a href="member.php?mod=connect" target="_blank">完善账号信息</a> <a href="member.php?mod=connect&amp;ac=bind" target="_blank">绑定已有账号</a>
</div>
<?php } else { ?>
<div class="y">
...
<?php } if($_G['setting']['allowviewuserthread'] !== false && (empty($personalnv['banitems']['topic']) && helper_access::check_module('forum'))) { ?>
...
<ul class="p_pop h_pop" id="plugin_menu" style="display: none"><?php if(isset($_G['setting']['plugins']['jsmenu']) && is_array($_G['setting']['plugins']['jsmenu'])) foreach($_G['setting']['plugins']['jsmenu'] as $module) { ?>     <?php if(in_array($module['adminid'], array(0, -1)) || ($module['adminid'] && $_G['adminid'] > 0 && $module['adminid'] >= $_G['adminid'])) { ?>
     <li><?php echo $module['url'];?></li>
     <?php } } ?>
</ul>
<?php } ?>
<?php echo $_G['setting']['menunavs'];?><?php $mnid = getcurrentnav();?><ul id="navs_menu" class="p_pop topnav_pop" style="display:none;"><?php if(isset($_G['setting']['navs']) && is_array($_G['setting']['navs'])) foreach($_G['setting']['navs'] as $nav) { $nav_showmenu = strpos($nav['nav'], 'onmouseover="showMenu(');?>    <?php $nav_navshow = strpos($nav['nav'], 'onmouseover="navShow(')?>    <?php if($nav_hidden !== false || $nav_navshow !== false) { $nav['nav'] = preg_replace("/onmouseover\=\"(.*?)\"/i", '',$nav['nav'])?>    <?php } ?>
    <?php if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?><li <?php echo $nav['nav'];?>></li><?php } } ?>
</ul>
<ul id="myspace_menu" class="p_pop" style="display:none;">
    <li><a href="home.php?mod=space">我的空间</a></li><?php if(isset($_G['setting']['mynavs']) && is_array($_G['setting']['mynavs'])) foreach($_G['setting']['mynavs'] as $nav) { if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?>
...
<?php if(!empty($_G['ols'][$space['uid']])) { ?>
<span title="在线" class="olicon vm"></span>&nbsp;
<?php } ?>
<span class="xw0">(UID: <?php echo $space['uid'];?><?php $isfriendinfo = 'home_friend_info_'.$space['uid'].'_'.$_G['uid'];?><?php if(!empty($_G[$isfriendinfo]['note'])) { ?>
...
<?php if(helper_access::check_module('friend')) { ?>
...
<?php } if($_G['setting']['allowviewuserthread'] !== false && helper_access::check_module('forum')) { ?>
...
<?php } ?>
<ul class="pf_l cl"><?php if(isset($profiles) && is_array($profiles)) foreach($profiles as $value) { ?><li><em><?php echo $value['title'];?></em><?php echo $value['value'];?></li>
<?php } ?>
</ul>
</div>
<?php if(CURMODULE == 'space') { ?>
...
<h2 class="mbn">勋章</h2>
<p class="md_ctrl">
<a href="home.php?mod=medal"><?php if(isset($space['medals']) && is_array($space['medals'])) foreach($space['medals'] as $medal) { ?><img src="<?php echo $medal['image'];?>" alt="<?php echo $medal['name'];?>" id="md_<?php echo $medal['medalid'];?>" onmouseover="showMenu({'ctrlid':this.id, 'menuid':'md_<?php echo $medal['medalid'];?>_menu', 'pos':'12!'});" />
<?php } ?>
</a>
</p>
</div><?php if(isset($space['medals']) && is_array($space['medals'])) foreach($space['medals'] as $medal) { ?><div id="md_<?php echo $medal['medalid'];?>_menu" class="tip tip_4" style="display: none;">
...
<?php } } if($_G['setting']['verify']['enabled']) { $showverify = true;?><?php if(isset($_G['setting']['verify']) && is_array($_G['setting']['verify'])) foreach($_G['setting']['verify'] as $vid => $verify) { if($verify['available']) { if($showverify) { ?>
<div class="pbm mbm bbda cl">
...
<div class="pbm mbm bbda cl">
<h2 class="mbn">管理以下版块</h2><?php if(isset($manage_forum) && is_array($manage_forum)) foreach($manage_forum as $key => $value) { ?><a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $key;?>" target="_blank"><?php echo $value;?></a> &nbsp;
<?php } ?>
</div>
<?php } if(!empty($groupcount)) { ?>
<div class="pbm mbm bbda cl">
<h2 class="mbn">已加入圈子</h2><?php if(isset($usergrouplist) && is_array($usergrouplist)) foreach($usergrouplist as $key => $value) { ?><a href="forum.php?mod=group&amp;fid=<?php echo $value['fid'];?>" target="_blank"><?php echo $value['name'];?></a> &nbsp;
<?php } ?>
</div>
<?php } ?>
<div class="pbm mbm bbda cl">
...
<ul>
<?php if($space['adminid'] > 0) { ?><li><em class="xg1">管理组&nbsp;&nbsp;</em><span style="color:<?php echo $space['admingroup']['color'];?>"><a href="home.php?mod=spacecp&amp;ac=usergroup&amp;gid=<?php echo $space['adminid'];?>" target="_blank"><?php echo $space['admingroup']['grouptitle'];?></a></span> <?php echo $space['admingroup']['icon'];?></li><?php } ?>
...
<?php if($space['oltime'] && !$_G['setting']['sessionclose']) { ?><li><em>在线时间</em><?php echo $space['oltime'];?> 小时</li><?php } ?>
<li><em>注册时间</em><?php echo $space['regdate'];?></li>
<li><em>最后访问</em><?php echo $space['lastvisit'];?></li>
<?php if($_G['uid'] == $space['uid'] || !empty($_G['group']['allowviewip'])) { ?>
<li><em>注册 IP</em><?php echo $space['regip'];?>:<?php echo $space['regport'];?> - <?php echo $space['regip_loc'];?></li>
...
<li><em>买家信用</em><a href="home.php?mod=space&amp;uid=<?php echo $space['uid'];?>&amp;do=trade&amp;view=eccredit<?php if($_G['setting']['homepagestyle']) { ?>&amp;sns=1<?php } ?>#sellcredit" target="_blank"><?php echo $space['buyercredit'];?> <img src="<?php echo STATICURL;?>image/traderank/buyer/<?php echo $space['buyerrank'];?>.gif" border="0" class="vm" /></a></li>
<?php } if($space['sellercredit']) { ?>
<li><em>卖家信用</em><a href="home.php?mod=space&amp;uid=<?php echo $space['uid'];?>&amp;do=trade&amp;view=eccredit<?php if($_G['setting']['homepagestyle']) { ?>&amp;sns=1<?php } ?>#buyercredit" target="_blank"><?php echo $space['sellercredit'];?> <img src="<?php echo STATICURL;?>image/traderank/seller/<?php echo $space['sellerrank'];?>.gif" border="0" class="vm" /></a></li>
<?php } ?>
<li><em>积分</em><?php echo $space['credits'];?></li><?php if(isset($_G['setting']['extcredits']) && is_array($_G['setting']['extcredits'])) foreach($_G['setting']['extcredits'] as $key => $value) { if($value['title']) { ?>
...
<th>操作理由</th>
<th width="15%">操作者</th>
</tr><?php if(isset($clist) && is_array($clist)) foreach($clist as $crime) { ?><tr>
<td>
...
<li class="ul_broadcast"><a href="home.php?mod=follow&amp;uid=<?php echo $space['uid'];?>&amp;do=view">查看广播</a></li>
...
<?php } if(helper_access::check_module('friend')) { if(!$isfriend) { ?>
...
<?php } if($_G['adminid'] == 1) { ?>
<li><a <?php if(helper_access::check_module('forum')) { ?>href="forum.php?mod=modcp&amp;action=thread&amp;op=post&amp;do=search&amp;searchsubmit=1&amp;users=<?php echo $encodeusername;?>" <?php } ?>id="umanageli" onmouseover="showMenu(this.id)" class="showmenu">内容管理</a></li>
<?php } ?>
</ul>
<?php if(checkperm('allowbanuser') || checkperm('allowedituser')) { ?>
...
<li><a href="admin.php?action=threads&amp;operation=group&amp;searchsubmit=1&amp;detail=1&amp;search=true&amp;fromumanage=1&amp;users=<?php echo $encodeusername;?>" target="_blank">圈子主题</a></li>
<li><a href="admin.php?action=prune&amp;searchsubmit=1&amp;detail=1&amp;operation=group&amp;fromumanage=1&amp;users=<?php echo $encodeusername;?>" target="_blank">圈子帖子</a></li>
<?php } ?>
</ul>
<?php } } ?>
</div>

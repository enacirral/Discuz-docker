...
0
|| checktplrefresh('./template/default/common/header.htm', './template/default/common/header_common.htm', 1700942702, '1', './data/template/1_1_common_header_forum_forumdisplay.tpl.php', './template/default', 'common/header_forum_forumdisplay')
|| checktplrefresh('./template/default/common/header.htm', './template/default/common/header_qmenu.htm', 1700942702, '1', './data/template/1_1_common_header_forum_forumdisplay.tpl.php', './template/default', 'common/header_forum_forumdisplay')
|| checktplrefresh('./template/default/common/header.htm', './template/default/common/pubsearchform.htm', 1700942702, '1', './data/template/1_1_common_header_forum_forumdisplay.tpl.php', './template/default', 'common/header_forum_forumdisplay')
;?>
<!DOCTYPE html>
<html>
<head>
...
<?php } if(!empty($_G['setting']['shortcut']) && $_G['member']['credits'] >= $_G['setting']['shortcut']) { ?>
<div id="shortcut">
...
<div class="wp">
<div class="z"><?php if(isset($_G['setting']['topnavs'][0]) && is_array($_G['setting']['topnavs'][0])) foreach($_G['setting']['topnavs'][0] as $nav) { if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?><?php echo $nav['code'];?><?php } } ?>
<?php if(!empty($_G['setting']['pluginhooks']['global_cpnav_extra1'])) echo $_G['setting']['pluginhooks']['global_cpnav_extra1'];?>
</div>
<div class="y">
<a id="switchblind" href="javascript:;" onclick="toggleBlind(this)" title="开启辅助访问" class="switchblind"></a>
<?php if(!empty($_G['setting']['pluginhooks']['global_cpnav_extra2'])) echo $_G['setting']['pluginhooks']['global_cpnav_extra2'];?><?php if(isset($_G['setting']['topnavs'][1]) && is_array($_G['setting']['topnavs'][1])) foreach($_G['setting']['topnavs'][1] as $nav) { if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?><?php echo $nav['code'];?><?php } } if(empty($_G['disabledwidthauto']) && $_G['setting']['switchwidthauto']) { ?>
...
<?php if(!IS_ROBOT) { if($_G['uid'] && !$_G['setting']['bbclosed'] && empty($_G['member']['freeze']) && $_G['member']['groupid'] != 5) { ?>
...
<?php if($_G['setting']['followstatus']) { ?>
...
<?php } } if($_G['member']['newprompt']) { if(isset($_G['member']['category_num']) && is_array($_G['member']['category_num'])) foreach($_G['member']['category_num'] as $key => $val) { ?><li><a href="home.php?mod=space&amp;do=notice&amp;view=<?php echo $key;?>"><em class="notice_<?php echo $key;?>"></em><?php echo lang('template', 'notice_'.$key); ?>(<span class="rq"><?php echo $val;?></span>)</a></li>
...
<?php if(!$_G['style']['defaultextstyle']) { ?><span class="sslct_btn" onclick="extstyle('')" title="默认"><i></i></span><?php } if(isset($_G['style']['extstyle']) && is_array($_G['style']['extstyle'])) foreach($_G['style']['extstyle'] as $extstyle) { ?><span class="sslct_btn" onclick="extstyle('<?php echo $extstyle[0];?>')" title="<?php echo $extstyle[1];?>"><i style='background:<?php echo $extstyle[2];?>'></i></span>
<?php } ?>
</div>
<?php } if($_G['uid']) { ?>
<ul id="myitem_menu" class="p_pop" style="display: none;">
<?php if($_G['setting']['forumstatus']) { ?><li><a href="home.php?mod=space&amp;do=thread&amp;view=me">帖子</a></li><?php } if($_G['setting']['favoritestatus']) { ?><li><a href="home.php?mod=space&amp;do=favorite&amp;view=me">收藏</a></li><?php } if($_G['setting']['friendstatus']) { ?><li><a href="home.php?mod=space&amp;do=friend">好友</a></li><?php } ?>
...
<?php if($_G['uid']) { ?>
<ul class="cl nav"><?php if(isset($_G['setting']['mynavs']) && is_array($_G['setting']['mynavs'])) foreach($_G['setting']['mynavs'] as $nav) { if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?>
...
<?php } if($_G['setting']['showfjump'] && $_G['setting']['forumstatus']) { ?><div id="fjump_menu" class="btda"></div><?php } ?>
...
<ul><?php if(isset($_G['setting']['navs']) && is_array($_G['setting']['navs'])) foreach($_G['setting']['navs'] as $nav) { if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?><li <?php if($mnid == $nav['navid'] || substr($_SERVER['REQUEST_URI'], 1) == str_replace('./', '', $nav['filename'])) { ?>class="a" <?php } ?><?php echo $nav['nav'];?>></li><?php } } ?>
</ul>
...
<div id="mu" class="cl">
<?php if($_G['setting']['subnavs']) { if(isset($_G['setting']['subnavs']) && is_array($_G['setting']['subnavs'])) foreach($_G['setting']['subnavs'] as $navid => $subnav) { if($_G['setting']['navsubhover'] || $mnid == $navid) { ?>
<ul class="cl <?php if($mnid == $navid) { ?>current<?php } ?>" id="snav_<?php echo $navid;?>"<?php if($mnid != $navid) { ?> style="display:none"<?php } ?>>
<?php echo $subnav;?>
</ul>
<?php } } } ?>
</div><?php echo adshow("subnavbanner/a_mu");?><?php if($_G['setting']['search']) { $slist = array();?><?php if($_G['setting']['forumstatus'] && $_G['fid'] && $_G['forum']['status'] != 3 && !(isset($mod) && $mod == 'group')) { ?><?php
$slist['forumfid'] = <<<EOF
...
EOF;
?><?php } if($_G['setting']['forumstatus'] && $_G['setting']['search']['forum']['status'] && ($_G['group']['allowsearch'] & 2 || $_G['adminid'] == 1)) { ?><?php
$slist['forum'] = <<<EOF
...
EOF;
?><?php } if($_G['setting']['friendstatus']) { ?><?php
$slist['user'] = <<<EOF
...
<?php if(!empty($searchparams['params'])) { if(isset($searchparams['params']) && is_array($searchparams['params'])) foreach($searchparams['params'] as $key => $value) { $srchotquery .= '&' . $key . '=' . rawurlencode($value);?><input type="hidden" name="<?php echo $key;?>" value="<?php echo $value;?>" />
<?php } ?>
...
<div id="scbar_hot">
<?php if($_G['setting']['forumstatus'] && $_G['setting']['srchhotkeywords']) { ?>
<strong class="xw1">热搜: </strong><?php if(isset($_G['setting']['srchhotkeywords']) && is_array($_G['setting']['srchhotkeywords'])) foreach($_G['setting']['srchhotkeywords'] as $val) { if($val=trim($val)) { $valenc=rawurlencode($val);?><?php

...
<div id="flk" class="y">
<p>
<?php if(!empty($_G['setting']['site_qq'])) { ?><a href="//wpa.qq.com/msgrd?v=3&amp;uin=<?php echo $_G['setting']['site_qq'];?>&amp;site=<?php echo $_G['setting']['bbname'];?>&amp;menu=yes&amp;from=discuz" target="_blank" title="QQ"><img src="<?php echo IMGDIR;?>/site_qq.jpg" alt="QQ" /></a><span class="pipe">|</span><?php } if(isset($_G['setting']['footernavs']) && is_array($_G['setting']['footernavs'])) foreach($_G['setting']['footernavs'] as $nav) { if(is_array($nav) && $nav['available'] && ($nav['type'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1)) ||
...
<?php if($_G['setting']['icp'] || !empty($_G['setting']['mps'])) { ?>( <?php if($_G['setting']['icp']) { ?><a href="https://beian.miit.gov.cn/" target="_blank"><?php echo $_G['setting']['icp'];?></a><?php } if(!empty($_G['setting']['mps'])) { if($_G['setting']['icp']) { ?><span class="pipe">|</span><?php } ?><a href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=<?php echo $_G['setting']['mpsid'];?>" target="_blank"><img width="14" height="14" src="<?php echo IMGDIR;?>/ico_mps.png" /><?php echo $_G['setting']['mps'];?></a><?php } ?> )<?php } ?>
...
<div id="frt">
<p>Powered by <strong><a href="https://www.discuz.vip/" target="_blank">Discuz!</a></strong> <em><?php echo $_G['setting']['version'];?></em><?php if(!empty($_G['setting']['boardlicensed'])) { ?> <a href="https://license.discuz.vip/?pid=1&amp;host=<?php echo $_SERVER['HTTP_HOST'];?>" target="_blank">Licensed</a><?php } ?></p>
...
<?php } if(!$_G['setting']['bbclosed'] && empty($_G['member']['freeze'] && $_G['member']['groupid'] != 5)) { if($_G['uid'] && !isset($_G['cookie']['checkpm'])) { ?>
...
<?php } if(!$_G['setting']['bbclosed'] && empty($_G['member']['freeze']) && $_G['member']['groupid'] != 5 && (empty($_G['setting']['disableipnotice']) || $_G['setting']['disableipnotice'] != 1) && $_G['uid'] && !empty($_G['cookie']['lip'])) { ?>
<div class="focus plugin" id="ip_notice"></div>
<script type="text/javascript">ipNotice();</script>
<?php } if(!empty($_G['member']['newprompt']) && (empty($_G['cookie']['promptstate_'.$_G['uid']]) || $_G['cookie']['promptstate_'.$_G['uid']] != $_G['member']['newprompt']) && $_GET['do'] != 'notice' && empty($_G['member']['freeze']) && $_G['member']['groupid'] != 5) { ?>
<script type="text/javascript">noticeTitle();</script>
<?php } if((!empty($_G['member']['newpm']) || !empty($_G['member']['newprompt'])) && empty($_G['cookie']['ignore_notice']) && empty($_G['member']['freeze']) && $_G['member']['groupid'] != 5) { ?>

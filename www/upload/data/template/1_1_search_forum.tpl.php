...
0
|| checktplrefresh('./template/default/search/forum.htm', './template/default/search/header.htm', 1700941548, '1', './data/template/1_1_search_forum.tpl.php', './template/default', 'search/forum')
|| checktplrefresh('./template/default/search/forum.htm', './template/default/search/pubsearch.htm', 1700941548, '1', './data/template/1_1_search_forum.tpl.php', './template/default', 'search/forum')
|| checktplrefresh('./template/default/search/forum.htm', './template/default/search/thread_list.htm', 1700941548, '1', './data/template/1_1_search_forum.tpl.php', './template/default', 'search/forum')
|| checktplrefresh('./template/default/search/forum.htm', './template/default/search/footer.htm', 1700941548, '1', './data/template/1_1_search_forum.tpl.php', './template/default', 'search/forum')
|| checktplrefresh('./template/default/search/forum.htm', './template/default/common/header_common.htm', 1700941548, '1', './data/template/1_1_search_forum.tpl.php', './template/default', 'search/forum')
;?>
<!DOCTYPE html>
<html>
<head>
...
<ul class="p_pop h_pop" id="navs_menu" style="display: none"><?php if(isset($_G['setting']['navs']) && is_array($_G['setting']['navs'])) foreach($_G['setting']['navs'] as $nav) { $nav_showmenu = strpos((isset($nav['nav']) ? $nav['nav'] : ''), 'onmouseover="showMenu(');?>    <?php $nav_navshow = strpos((isset($nav['nav']) ? $nav['nav'] : ''), 'onmouseover="navShow(')?>    <?php if(!isset($nav_hidden) || $nav_hidden !== false || !isset($nav_navshow) || $nav_navshow !== false) { $nav['nav'] = preg_replace("/onmouseover\=\"(.*?)\"/i", '',(isset($nav['nav']) ? $nav['nav'] : ''))?>    <?php } if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?><li <?php echo $nav['nav'];?>></li><?php } } ?>
</ul>
<?php } ?>

<ul id="myspace_menu" class="p_pop" style="display:none;"><?php if(isset($_G['setting']['mynavs']) && is_array($_G['setting']['mynavs'])) foreach($_G['setting']['mynavs'] as $nav) { if(is_array($nav) && $nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?>
...
<input type="hidden" name="formhash" value="<?php echo FORMHASH;?>" /><?php $keywordenc = $keyword ? rawurlencode($keyword) : '';?><?php if($searchid || (!empty($_GET['adv']) && CURMODULE == 'forum')) { ?>
<table id="scform" class="mbm" cellspacing="0" cellpadding="0">
<tr>
<td><h1><a href="search.php" title="<?php echo $_G['setting']['bbname'];?>"><?php echo $_G['style']['searchlogo'];?></a></h1></td>
<td>
...
<?php if(CURMODULE == 'forum') { ?>
<a href="search.php?mod=forum&amp;adv=yes<?php echo $advextra;?>">高级</a>
<?php } ?>
</span>
...
>文章</a>
EOF;
?><?php } if(helper_access::check_module('forum') && $_G['setting']['search']['forum']['status'] && ($_G['group']['allowsearch'] & 2 || $_G['adminid'] == 1)) { ?><?php
$slist['forum'] = <<<EOF
...
>淘帖</a>
EOF;
?><?php } if(helper_access::check_module('friend') ) { ?><?php
$slist['user'] = <<<EOF
...
<tr>
<td class="td_srchtxt"><input type="text" id="scform_srchtxt" name="srchtxt" size="45" maxlength="40" value="<?php echo $keyword;?>" x-webkit-speech speech /><script type="text/javascript">initSearchmenu('scform_srchtxt');$('scform_srchtxt').focus();</script></td>
...
<div class="hm mtw ptw pbw"><h1 class="mtw ptw"><a href="./" title="<?php echo $_G['setting']['bbname'];?>"><?php echo $_G['style']['searchlogo'];?></a></h1></div>
...
>淘帖</a>
EOF;
?><?php } echo implode("", $slist);; if(helper_access::check_module('friend') ) { ?><a href="search.php?mod=user<?php if($keyword) { ?>&amp;srchtxt=<?php echo $keywordenc;?>&amp;searchsubmit=yes<?php } ?>"<?php if(CURMODULE == 'user') { ?> class="a"<?php } ?>>用户</a><?php } ?>
</td>
</tr>
<tr>
<td>
...
<tr>
<td class="td_srchtxt"><input type="text" id="scform_srchtxt" name="srchtxt" size="65" maxlength="40" value="<?php echo $keyword;?>" /><script type="text/javascript">initSearchmenu('scform_srchtxt');$('scform_srchtxt').focus();</script></td>
...
<?php } ?>
<ul><?php if(isset($threadlist) && is_array($threadlist)) foreach($threadlist as $thread) { ?><li class="pbw" id="<?php echo $thread['tid'];?>">
<h3 class="xs3">
...
<?php } else { if($_G['forum']['ismoderator']) { ?><a href="home.php?mod=space&amp;uid=<?php echo $thread['authorid'];?>" target="_blank"><?php echo $_G['setting']['anonymoustext'];?></a><?php } else { ?><?php echo $_G['setting']['anonymoustext'];?><?php } } ?>
</span>
 -
...
<?php if($modfid) { ?>
<script src="<?php echo $_G['setting']['jspath'];?>forum_moderate.js?<?php echo VERHASH;?>" type="text/javascript"></script><?php include template('forum/topicadmin_modlayer'); ?></form>
<?php } ?>
</div>
...
<em>Powered by <strong><a href="https://www.discuz.vip/" target="_blank">Discuz!</a></strong> <em><?php echo $_G['setting']['version'];?></em><?php if(!empty($_G['setting']['boardlicensed'])) { ?> <a href="https://license.discuz.vip/?pid=1&amp;host=<?php echo $_SERVER['HTTP_HOST'];?>" target="_blank">Licensed</a><?php } ?></em> &nbsp;
<em>&copy; 2001-2023 <a href="https://code.dismall.com/" target="_blank">Discuz! Team</a>.</em>
<span class="pipe">|</span><?php if(isset($_G['setting']['footernavs']) && is_array($_G['setting']['footernavs'])) foreach($_G['setting']['footernavs'] as $nav) { if(is_array($nav) && $nav['available'] && ($nav['type'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1)) ||
...
<?php if($_G['setting']['icp'] || $_G['setting']['mps']) { ?>( <?php if($_G['setting']['icp']) { ?><a href="https://beian.miit.gov.cn/" target="_blank"><?php echo $_G['setting']['icp'];?></a><?php } if($_G['setting']['mps']) { if($_G['setting']['icp']) { ?><span class="pipe">|</span><?php } ?><a href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=<?php echo $_G['setting']['mpsid'];?>" target="_blank"><img width="14" height="14" src="<?php echo IMGDIR;?>/ico_mps.png" /><?php echo $_G['setting']['mps'];?></a><?php } ?> )<?php } ?>

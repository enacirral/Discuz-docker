...
0
|| checktplrefresh('./template/default/member/login.htm', './template/default/common/seccheck.htm', 1700941701, '1', './data/template/1_1_member_login.tpl.php', './template/default', 'member/login')
...
<td><input type="text" name="username" id="username_<?php echo $loginhash;?>" autocomplete="off" size="30" class="px p_fre" value="<?php echo $username;?>" /></td>
...
<td><input type="password" id="password3_<?php echo $loginhash;?>" name="password" onfocus="clearpwd()" size="30" class="px p_fre" /></td>
...
<?php } if(empty($_GET['auth']) || $questionexist || $seccodecheck) { ?>
<div class="rfm">
<table>
<tr>
<th>安全提问:</th>
<td><select id="loginquestionid_<?php echo $loginhash;?>" width="213" name="questionid"<?php if(!$questionexist || $seccodecheck) { ?> onchange="if($('loginquestionid_<?php echo $loginhash;?>').value > 0) {$('loginanswer_row_<?php echo $loginhash;?>').style.display='';} else {$('loginanswer_row_<?php echo $loginhash;?>').style.display='none';}"<?php } ?>>
<option value="0"><?php if($questionexist && !$seccodecheck) { ?>安全提问<?php } else { ?>安全提问(未设置请忽略)<?php } ?></option>
...
<div class="rfm" id="loginanswer_row_<?php echo $loginhash;?>" <?php if(!$questionexist || $seccodecheck) { ?> style="display:none"<?php } ?>>
<table>
<tr>
<th>答案:</th>
<td><input type="text" name="answer" id="loginanswer_<?php echo $loginhash;?>" autocomplete="off" size="30" class="px p_fre" /></td>
</tr>
</table>
</div>
<?php } if($seccodecheck || $secqaacheck) { ?><?php
$sectpl = <<<EOF
...
<tr>
<th></th>
<td><label for="cookietime_<?php echo $loginhash;?>"><input type="checkbox" class="pc" name="cookietime" id="cookietime_<?php echo $loginhash;?>" value="2592000" <?php echo $cookietimecheck;?> />自动登录</label></td>
</tr>
</table>
</div>
...
<td>
<?php if($this->setting['sitemessage']['login'] && empty($_GET['infloat'])) { ?><a href="javascript:;" id="custominfo_login_<?php echo $loginhash;?>" class="y">&nbsp;<i class="fico-info fic4 fc-p" alt="帮助" class="vm"></i></a><?php } ?>								
</td>
</tr>
</table>
</div>
